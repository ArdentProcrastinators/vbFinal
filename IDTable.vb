Public Class IDTable
    ...
    Public Shared Function IDAbility(c As Card) As Integer
        ...
        Return 0
    End Function
    'Adds mana to the manaPool when a land is tapped and displays that it is tapped
    Public Shared Sub landUse(c As Card)
        ...
        End If
    End Sub
    'Actually uses the code to cause the ability to happen
    ...
    Public Shared Function TapImage(c As Card)
        'Changes the land image to tapped
        Select Case c.ID
            Case 3
    ...
    'Recreates a card object on the board
    ...
    End Sub
    'Sub used for instantiating new creature, saves on alot of code
    Public Shared Sub PlayCreature(c As Card)
        'If mana is payed
        If PayMana(c) Then
            'If radiant turn
            If Form1.RadiantTurn Then
                'Generates new creature and removes the card from the hand
                Form1.GenerateCreature(c.ID, True)
                Form1.RadiantHandInfo.Remove(c)
                c.Dispose()
                Form1.UpdateHand(Form1.RadiantTurn)
    ...
    'Generates the land for the right player and removes the card from the hand
